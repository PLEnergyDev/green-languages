...
pub enum Rapl {
    ...
    /// Package energy.
    PKG,
    /// Directly attached main memory energy.
    DRAM,
...
impl Rapl {
    ...
    pub fn get_config(&self) -> u64 {
        ...
        // Fallback values (these may not work on all systems)
        match self {
            Rapl::CORES => 0x01,
            Rapl::PKG => 0x02,
            Rapl::DRAM => 0x03,
    ...
    pub fn get_pmu_type() -> u32 {
        ...
        for path in &type_paths {
            if let Ok(contents) = read_to_string(path) {
                if let Ok(pmu_type) = contents.trim().parse::<u32>() {
                    return pmu_type;
                }
            }
        }
    ...
    /// Convert raw energy event values to Joules using energy event scales
    pub fn to_joules(&self, raw_value: u64) -> io::Result<f64> {
        let energy_unit = self.get_energy_unit()?;
        Ok(raw_value as f64 * energy_unit)
    }

    /// Check if the requested energy event is available on the system
    pub fn is_available(&self) -> bool {
        self.get_energy_unit().is_ok()
    }
}

impl From<Rapl> for Event {
    fn from(rapl: Rapl) -> Event {
        Event::Rapl(rapl)
    }
}
