...
debug: $(M_SO_DEBUG) $(M_A_DEBUG)
	...
	sudo ln -sf green-languages $(GL_BIN_DIR)/$(GL_BIN_ALIAS)
	for script in script/*.py; do \
		if [ "$$(basename $$script)" = "__main__.py" ]; then \
			sudo install -m 755 $$script $(GL_BIN_DIR)/gls; \
		else \
			sudo install -m 644 $$script $(GL_LIB_DIR)/$$(basename $$script); \
		fi \
	done
...
uninstall:
	...
	sudo rm -f $(GL_BIN_DIR)/$(GL_BIN_ALIAS)
	sudo rm -f $(GL_BIN_DIR)/gls
