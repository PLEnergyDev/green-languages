...
pub fn run(args: MeasureArgs) -> Result<(), Box<dyn std::error::Error>> {
    ...
    // Power events
    // let mut pkg = DynEvent::new("power", "energy-pkg", &mut group)?;
    // let mut psys = DynEvent::new("power", "energy-psys", &mut group)?;
    // let mut gpu = DynEvent::new("power", "energy-gpu", &mut group)?;

    // C-state events
    let mut c6_residency = DynEvent::new("cstate_core", "c6-residency", &mut group)?;
    let mut c8_residency = DynEvent::new("cstate_pkg", "c8-residency", &mut group)?;
    ...
    println!("Measured for {:.6}s", time_running.as_secs_f64());
    // println!();
    // println!("Power:");
    // println!("  Package: {:.3} {}", pkg.read()?, pkg.unit().unwrap_or(""));
    // println!(
    //     "  Psys:    {:.3} {}",
    //     psys.read()?,
    //     psys.unit().unwrap_or("")
    // );
    // println!("  GPU:     {:.3} {}", gpu.read()?, gpu.unit().unwrap_or(""));
    println!();
    println!("C-States:");
    println!(
        "  C6 Core: {:.3} {}",
        c6_residency.read()?,
        c6_residency.unit().unwrap_or("")
    );
    println!(
        "  C8 Pkg:  {:.3} {}",
        c8_residency.read()?,
        c8_residency.unit().unwrap_or("")
    );
